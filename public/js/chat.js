...
// 	var listenerGender = document.getElementById("listener-gender").value;
// 	console.log(listenerGender)
// });
...
function sortMessages(){
	...
	msgList = document.getElementsByClassName("message-left");
	for (let i = 0; i < msgList.length; i++){
		let auth = document.getElementsByClassName("message-author")[i];
		authContent = auth.textContent;
		if (authContent === "listener"){
			console.log(authContent)
			listenerMsgs.push(auth.parentNode.parentNode);

		} else {
			console.log(authContent)
			speakerMsgs.push(auth.parentNode.parentNode);
		}
	}

	for (let i = 0; i < 10; i++){
		console.log(listenerMsgs[i]);
		console.log(speakerMsgs[i].classList.add("message-right"))
	}
